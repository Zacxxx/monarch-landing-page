...
const ProjectShowcaseSection: React.FC<ProjectShowcaseSectionProps> = ({ id }) => {
  const titleRef = useScrollAnimation({ animationClass: 'scroll-animate-slide-right', triggerOnce: true });
  const textContentRef = useScrollAnimation({ animationClass: 'scroll-animate-fade-in', delay: 200, triggerOnce: true });
  const imageRef = useScrollAnimation({ animationClass: 'scroll-animate-slide-left', delay: 100, triggerOnce: true });
  const buttonRef = useScrollAnimation({ animationClass: 'scroll-animate-fade-in scroll-animate-scale-up', delay: 400, triggerOnce: true });
    ...
    <Section id={id} contentAlignment="left" className="bg-anim-project-showcase">
      <div className="max-w-4xl w-full">
        ...
        <div className="grid md:grid-cols-2 gap-8 sm:gap-12 items-center">
          <div ref={imageRef} className="relative aspect-video bg-secondary rounded-lg overflow-hidden shadow-2xl group">
